...
export class AccountService {
  ...
  login(email: string, password: string, dob: string) {
    return this.http
      .post<Account>(`${baseUrl}/authenticate`, { email, password, dob }, { withCredentials: true })
      .pipe(
        map((account) => {
  ...
  refreshToken() {
    return this.http
      .post<any>(`${baseUrl}/refresh-token`, {/* headers: headers */}, { withCredentials: true })
      .pipe(
        map((account) => {
          this.accountSubject.next(account);
  ...
  forgotPassword(request: ForgotPasswordRequest) {
    return this.http.post(`${baseUrl}/forgot-password`, request);
  }
  ...
  resetPassword(request: ResetPasswordRequest) {
    return this.http.post(`${baseUrl}/reset-password`, request);
  }
  ...
  getAvailablePoolElementsForAccount(id: any) {
    return this.http.get<SchedulePoolElements>(
      `${baseUrl}/available_pool-elements-for-account/${id}`,
    );
  }
  ...
  updateSchedule(id: any, schedule: Schedule) {
    return this.http.post(`${baseUrl}/update-schedule/${id}`, schedule).pipe(
      map((account: any) => {
  ...
  deletePoolElement(id: string, email: string, userFunction: string) {
    return this.http.post<SchedulePoolElement>(
      `${baseUrl}/remove-pool-element/${id}/${email}/${userFunction}`,
      '',
    );
  }
